...
class q2DStructure(Atoms):
    ...
    def twist(self, m, n, interlayer_distance=11.0, vacuum=12.0, **kwargs):
        ...
        **kwargs : dict
        ...
        """
        if self.structure_type != 'monolayer':
            raise ValueError(
                f"twist() method is only available for monolayer structures. "
                f"Current structure_type: {self.structure_type}"
            )
        ...
        monolayer = adapter.get_structure(self)
        ...
        slab_bottom = monolayer.copy()
        slab_bottom.make_supercell([[m, n, 0],
                                    [-n, m, 0],
                                    [0, 0, 1]])
        
        # === Top layer ===
        slab_top = monolayer.copy()
