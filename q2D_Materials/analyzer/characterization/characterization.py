...
def find_paths(
    ...
    Uses shared graph utilities for consistency with molecular graph analysis.
    ...
    """
    from ..utils.graph_utils import find_all_paths
    
    graph = analyzer.get_graph()
    return find_all_paths(graph, source, target, max_length)
...
def get_subgraph(analyzer: "q2D_analyzer", node_ids: List[str]) -> nx.Graph:
    ...
    """
    from ..utils.graph_utils import extract_subgraph
