...
def create_twisted_bilayer(mono1, mono2, m, n, interlayer_distance=11.0, vacuum=12.0, **kwargs):
    ...
    ----------
    mono1 : q2DStructure
        First monolayer structure (must have structure_type='monolayer')
        Must have at least 2x2 octahedra in the XY plane.
    mono2 : q2DStructure
        Second monolayer structure (must have structure_type='monolayer')
        Must have at least 2x2 octahedra in the XY plane.
    m : int
    ...
    **kwargs : dict
    ...
    for i, mono in enumerate([mono1, mono2], 1):
        ...
        supercell_size = getattr(mono, 'supercell_size', None)
            ...
            if isinstance(supercell_size, (list, tuple)) and len(supercell_size) >= 2:
                ...
                if nx < 2 or ny < 2:
                    raise ValueError(
                        ...
                        f"Structure {i} supercell_size={supercell_size}. "
    ...
    # Check for orthorhombic cells (a ≠ b) from Glazer tilting
    # For twist to work with PBC, the cells need to be approximately square in XY
    def ensure_square_cell(pmg_structure, name):
        cell_lengths = pmg_structure.lattice.lengths
        ...
        if ortho_ratio > ortho_tolerance:
            ...
            old_lattice = pmg_structure.lattice
            new_matrix = np.array([
                [avg_a, 0, 0],
                [0, avg_a, 0],
            ...
            frac_coords = pmg_structure.frac_coords
            species = [site.species for site in pmg_structure]
            return Structure(new_lattice, species, frac_coords)
        return pmg_structure

    mono1_pmg = ensure_square_cell(mono1_pmg, "mono1")
    mono2_pmg = ensure_square_cell(mono2_pmg, "mono2")
    ...
    # Calculate Z extents to properly position layers
    bottom_z_max = np.max(bottom_coords[:, 2])
    top_z_min = np.min(top_coords[:, 2])
    top_z_max = np.max(top_coords[:, 2])

    # Apply rotation to top layer (vectorized)
    top_coords_rotated = top_coords @ rot_matrix.T

    # Calculate proper Z offset: top layer should be positioned so that
    # the distance between bottom layer's top surface and top layer's bottom surface
    # equals interlayer_distance
    z_offset = bottom_z_max - top_z_min + interlayer_distance
    top_coords_rotated[:, 2] += z_offset  # Vectorized z-shift
    ...
    # Add supercell_size to metadata if it exists
    if hasattr(mono1, 'supercell_size') and mono1.supercell_size is not None:
        new_metadata['supercell_size'] = mono1.supercell_size
    ...
    return q2DStructure(
        ...
        A_ions=mono1.A_ions,  # Use first monolayer's composition as primary
        B_ions=mono1.B_ions,
        X_ions=mono1.X_ions,
        spacer=getattr(mono1, 'spacer', None),
