...
dj = q2d.create_structure(
    ...
    spacer="[NH3+]CCCC[NH3+]",  # SMILES or Atoms; list cycles
    glazer_angles=[0, 0, 3],
    glazer_pattern=["0", "0", "+"],
)
```
...
## Tips
- Provide `spacer` as SMILES or pre‑built ASE `Atoms`; lists cycle across S# labels.
...
## Troubleshooting
- Spacer missing: ensure the template has S# sites on two consecutive layers and `spacer` is a molecule (not a bare atom).
